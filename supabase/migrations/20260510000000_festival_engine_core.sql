-- ============================================================
-- Migration: Festival Engine Core (idempotente)
-- Re-executar é seguro: todos os passos usam IF NOT EXISTS / DO blocks
-- ============================================================

-- ── 1. Enums (ignorados se já existirem) ────────────────────
...
-- (só renomeia se 'categories' ainda existir)
...
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS modality                   category_modality NOT NULL DEFAULT 'classical';
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS formation                  formation_type NOT NULL DEFAULT 'solo';
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS min_dancers                INTEGER NOT NULL DEFAULT 1;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS max_dancers                INTEGER NOT NULL DEFAULT 1;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS min_age                    INTEGER;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS max_age                    INTEGER;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS max_duration_seconds_strict BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS is_competitive             BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS inscription_fee_per_dancer DECIMAL(10,2) NOT NULL DEFAULT 0;
ALTER TABLE festival_categories ADD COLUMN IF NOT EXISTS updated_at                 TIMESTAMPTZ NOT NULL DEFAULT NOW();
...
-- Tornar delegation_id NOT NULL e remover school_id
-- (seguro: só age se a coluna ainda for nullable / existir)
DO $$ BEGIN
...
-- ── 8. Índices (ignorados se já existirem) ───────────────────
...
CREATE INDEX IF NOT EXISTS idx_fest_cat_festival_id       ON festival_categories(festival_id);
...
CREATE INDEX IF NOT EXISTS idx_choreographies_category_id ON choreographies(category_id);
...
-- Remover novas políticas caso a migration já tenha sido parcialmente aplicada
...
-- ── 10. Novas políticas RLS: festivals ───────────────────────
...
-- ── 15. RLS: invoices + invoice_items (writes faltantes) ─────
